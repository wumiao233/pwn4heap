...
#define CHUNKNAMELEN 0x20

struct chunk
{
    char name[CHUNKNAMELEN];
    uint64_t size;
    uint64_t* addr;
};
...
void add_malloc()
{
    ...
    write_content("Please input the chunk name > ");
    read(0, chunks[index].name, CHUNKNAMELEN);
...
void show()
{
    ...
    uint64_t index = read_uint64();
    puts("Name: ");
    write(1, chunks[index].name, CHUNKNAMELEN);
    puts("Content: ");
